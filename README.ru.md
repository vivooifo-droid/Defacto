...
**Что нового в v0.42:**
- **Исправления указателей** — исправлен lexer, parser и codegen для указателей
- **Присваивание разыменования** — `*ptr = value` теперь работает корректно
- **Поддержка 64-bit macOS** — правильная обработка указателей в terminal режиме
- **Авто-определение типов** — `display{}` теперь работает с типами i32/i64/u8

**Что нового в v0.40:**
- **Операторы сравнения** — `!=`, `<`, `>`, `<=`, `>=`
- **Возврат из функций** — `return{value}`
- **While циклы** — `while x < y { ... }`
- **Импорт библиотек** — `Import{libname}`
- **Установка через Homebrew** — `brew install defacto`
