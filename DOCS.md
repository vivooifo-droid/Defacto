# Defacto v0.25 Documentation

Status: pre-alpha. The language, syntax, and runtime are unstable and may change at any time.

This document is the complete language reference for the current implementation.
...
Every file must start and end with these directives:
...
- `#Mainprogramm.start` and `#Mainprogramm.end` are required.
- `#NO_RUNTIME` is required for bare-metal mode.
- `#SAFE` is currently reserved. Memory checks are always enabled.
- `#INTERRUPT {num} == func` is parsed but not emitted yet.

## Sections

Code must be inside sections:
...
- Negative numbers are not supported as literals. Use `(0 - 1)` or a temp variable.
- Array access in expressions must use `arr[idx]` where `idx` is a number, variable, or register.
...
- Index must be a number, variable, or register.
...
- Only `==` is supported.
- Left and right sides must be a single token: number, variable, or register.
- No else.
...
Registers are addressed as `#R1..#R16`:
...
- Register mapping is fixed to x86-32 registers.
- `#STATIC` is parsed but not emitted.
